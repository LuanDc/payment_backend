...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/payment_backend ./
...
CMD ["sh", "-c", "/app/bin/payment_backend eval PaymentBackend.Release.migrate && /app/bin/server"]
